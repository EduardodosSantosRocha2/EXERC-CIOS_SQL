...
SELECT C.nome, C.cidade
		FROM quarto AS Q, tipoquarto AS T,cliente AS C,reserva AS R
			WHERE Q.idtipo = T.id_tipoquarto AND C.ref_cliente = R.ref_cliente AND R.num = Q.num
			AND C.cidade = 'Rio de Janeiro' AND T.designacao = 'Quarto Familiar' AND R.regime = 'MP'
UNION 

SELECT C.nome, C.cidade
		FROM quarto AS Q, tipoquarto AS T,cliente AS C,reserva AS R
			WHERE Q.idtipo = T.id_tipoquarto AND C.ref_cliente = R.ref_cliente AND R.num = Q.num
			AND C.cidade = 'Buenos Aires' AND T.designacao LIKE 'Suite%' AND R.regime = 'MP'
...
-- Ou 
	SELECT C.nome, C.ref_cliente
		FROM quarto AS Q, tipoquarto AS T,cliente AS C,reserva AS R
			WHERE Q.idtipo = T.id_tipoquarto AND C.ref_cliente = R.ref_cliente AND R.num = Q.num
			AND Q.andar = 2 AND C.nome NOT IN(SELECT C.nome
													FROM quarto AS Q, tipoquarto AS T,cliente AS C,reserva AS R
															WHERE Q.idtipo = T.id_tipoquarto AND C.ref_cliente = R.ref_cliente AND R.num = Q.num
															AND Q.andar = 1);
	
-- EX33
	SELECT DISTINCT C.nome, C.ref_cliente, Q.num AS Quarto_numero
		FROM quarto AS Q,cliente AS C,reserva AS R
			WHERE andar = 2 ORDER BY C.nome;
	
-- EX34
	
	SELECT *
		FROM quarto AS Q,reserva AS R
			WHERE Q.num = R.num AND R.numeropessoas = 3;
-- EX35
	SELECT *
		FROM quarto AS Q,reserva AS R
			WHERE Q.num = R.num
	...
	SELECT *
		FROM quarto AS Q,reserva AS R
			WHERE Q.num = R.num AND R.numeropessoas = 3;
-- EX36
	SELECT DISTINCT C.nome,C.ref_cliente,C.cidade,C.pais,Q.num as quarto_numero, Q.andar, Q.idtipo
		FROM quarto AS Q, tipoquarto AS T,cliente AS C,reserva AS R
			WHERE  T.id_tipoquarto = Q.idtipo AND designacao LIKE 'Suite%' ORDER BY C.nome;